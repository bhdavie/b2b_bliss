...
(function () {
  ...
  var CONFIG = {
    ...
    detailsStep: {
      /**
       * PAGE GATE. The details block mounts on this path and nowhere else.
       *
       * There was no explicit gate until now. Page detection used to be an
       * accident of the anchor: "Total Reservation" appears only on checkout,
       * so matching that text implicitly meant "we are on checkout". Moving the
       * anchor to <ibe-ct-small-rate> removed that accident without replacing
       * it, and that element also renders on the accommodations page, on the
       * selected-room summary bar. The block mounted there and forced the room
       * name to wrap one character per line.
       *
       * Substring test, so /book/checkout and anything below it match while
       * /book/accommodations does not. Set to null to disable the gate.
       */
      pathRe: /checkout/i,
   ...
   * have a box before anything mounts (see findDetailsPlacement and hasBox), so
   * a zero measurement here means the placement targets themselves are wrong
   * and the fix belongs in CONFIG.detailsStep.placementTargets, not in a
   * runtime scramble that lands the block somewhere arbitrary.
   */
  ...
  function findDetailsRow() {
    ...
    // required to have a box, so returning it would hand callers an unlaid-out
    // node. A null row is not fatal; the row is now only reported by the probe
    // and used as the staleness key on the mount guard.
    return null;
  }
  ...
  /** The current pathname, or "" when it cannot be read. */
  ...
  /**
   * Are we on the checkout step?
   *
   * The explicit replacement for the page detection that used to fall out of
   * the "Total Reservation" text match. Everything about the details block is
   * checkout-specific, so this is checked before any of it runs.
   *
   * An unconfigured pathRe means no gate, which is deliberate: a property whose
   * engine uses a different path should degrade to the old behaviour rather
   * than silently never mounting.
   */
  function onCheckoutPath() {
    var cfg = CONFIG.detailsStep;
    if (!cfg || !cfg.pathRe) return true;
    return cfg.pathRe.test(currentPath());
  }

  function findDetailsPlacement() {
    var cfg = CONFIG.detailsStep;
    ...
    // NO ROW FALLBACK. This used to fall through to chooseDetailsSlot, which
    // inserted next to whatever ancestor of the amount anchor happened to have
    // a box. It never produced a correct placement, and on the accommodations
    // page it put the block inside div.product-selector-rate, the selected-room
    // summary bar, which forced the room name to wrap one character per line.
    //
    // A configured placement target is now the only way in. If none resolves,
    // this page is not one we know how to place on and we do not mount.
    return null;
  }
  ...
  function startHostWatchdog() {
    ...
    watchdogTimer = window.setInterval(function () {
      ...
      // Path first, and cheaper than the tree walk below. <ibe-ct-small-rate>
      // also renders on the accommodations page, so without this the watchdog
      // would re-sync twice a second there, chasing a block that is correctly
      // never going to mount.
      if (!onCheckoutPath()) return;
  ...
  /**
   * Unmounts the tracked details block and forgets it. Safe with null, and
   * safe to call when the host is already detached.
   */
  function dropDetailsTrigger(existing) {
    if (!existing) return;
  ...
  function ensureDetailsTrigger() {
    ...
    // PAGE GATE, before anything else runs. Everything below is checkout-only,
    // and on the accommodations page the anchor resolves perfectly well against
    // the selected-room summary bar, so nothing further down can be trusted to
    // notice we are on the wrong page.
    if (!onCheckoutPath()) {
      detailsBail =
        'BAILED at page gate (pathname "' + currentPath() + '" does not match ' + cfg.pathRe + ")";
      cancelDetailsRecheck();
      dropDetailsTrigger(existing);
      return;
    }
    ...
    // PLACEMENT IS THE GATE. A configured target must resolve; there is no
    // longer a fallback that will place the block somewhere approximate.
    var placement = findDetailsPlacement();
    ...
    if (!placement) {
      ...
      detailsBail =
        "BAILED at placement gate (no placementTarget resolved; anchor " +
        (label ? "resolved" : "did not resolve") +
